...
hist_long%>% subset(Month %in% c(-2:12))%>%
  ...
  theme_minimal() +
  geom_line(alpha=.8,aes(x = Month), na.rm=TRUE)+  geom_point(alpha=.9,aes(x = Month)) +
  theme(legend.position="bottom") +scale_color_d3()+scale_fill_d3()+ scale_x_continuous(breaks=c(-2:12), labels =c("Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar","Apr"))+  labs(colour = "",fill="", x="Months",y="Forecasted & Observed Change")+facet_wrap(~domain, scales = "free", nrow = 4, labeller=labeller(domain=labels))+ geom_line(data=dat_longX,alpha=.09,aes(x = Month, group=team_name), na.rm=TRUE)+  #geom_point(data = dat_longX,alpha=.1,aes(x = Month0, group=team_name)) +
  theme(axis.text.x = element_text(angle=45, vjust=.5, hjust=1, size=rel(0.5)))+geom_smooth(data=dat_long %>% subset(!is.na(Method.code)& Method.code!="Ground Truth"& Method.code!="Naive-linear"&Method.code!="Naive-rfw" &Method.code!="Lay People"& Month %in% c(-2:12)), aes(x = Month, y = value, colour = phaseF, fill=phaseF),method = "loess") 

histbench<-hist_long%>%subset(Month %in% c(-1))
#with last data point as a horizontal line
dat_long %>% subset(!is.na(Method.code)& Method.code!="Ground Truth"& Method.code!="Naive-linear"&Method.code!="Naive-rfw" &Method.code!="Lay People"& Month %in% c(1:12)) %>% 
   ggplot(aes(x = Month0, y = value, colour = phaseF, fill=phaseF))+geom_line(data=dat_longX,alpha=.09,aes(x = Month0, group=team_name), na.rm=TRUE)+  #geom_point(data = dat_longX,alpha=.1,aes(x = Month0, group=team_name)) +
  geom_smooth(aes(x = Month0, y = value, colour = phaseF, fill=phaseF),method = "loess") +  
  geom_line(data=objective,alpha=.8,aes(x = Month0, group=team_name), na.rm=TRUE)+  geom_point(data = objective,alpha=.9,aes(x = Month0)) + scale_color_d3()+scale_fill_d3()+ xlim(0,12)+
    geom_hline(aes(yintercept = value),histbench,linetype="dotted",color="black")+#get the data right  - one per domain.
  facet_wrap(~domain, scales = "free", nrow = 4, labeller=labeller(domain=labels))+theme_minimal()+scale_x_continuous(breaks=c(0:11), labels =c("May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar","Apr"))+  labs(colour = "",fill="", x="Months",y="Forecasted & Observed Change")+theme(axis.text.x = element_text(angle=45, vjust=.5, hjust=1, size=rel(0.5)))+ theme(legend.position="bottom") 
```
...
data.phase1.MASE.together$Wave<-"First Tournament"
...
data.phase2.MASE.together$Wave<-"Second Tournament"
...
#export_summs(model.bothTournaments.COVs, scale = F, robust = "HC1", to.file = "docx")
...
eff_size(emmeans(model.layVSsci.phase1,specs = trt.vs.ctrl ~isExpert.factor|domain,  type="response",adjust = "fdr") 
, sigma = sigma(model.layVSsci.phase1), edf = 1796) #get Cohen's D effect sizes
...
updated.analysis <-lmer(log(MASE1_w1)~revised +(1|ResponseId),data=expert_dat)
print(updated.analysis)
emmeans(updated.analysis,~revised, scale="response")
anova.updated  <- car::Anova(updated.analysis,type="III")

print(anova.updated) #no difference

  #using MASE2

updated.analysis2 <-lmer(log(MASE1_w2)~revised+(1|ResponseId),data=expert_dat)
print(updated.analysis2)
emmeans(updated.analysis2,~revised, scale="response")
