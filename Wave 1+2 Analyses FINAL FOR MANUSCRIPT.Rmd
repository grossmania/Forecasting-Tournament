...
emmeans(model.phase2.topBenchmark.ratio,~domain, type="response")%>% rbind(adjust="mvt")


plot.t2.topBenchmark<-plot(emmeans(model.phase2.topBenchmark.ratio,~domain, type="response")%>% rbind(adjust="mvt"),comparisons=F, color="black")+scale_y_discrete(labels=labeling, name="Top Naive Benchmark")+geom_vline(xintercept =1, linetype='dashed', color='black',14)+theme_minimal()+  labs(x="",shape="",color="", title="Tournament 2 (Nov 2020)")
...
emtrends(model.phase2.predicted.by.phase1.academ,"domain",var = "MASE1_w1"                        ) %>% rbind(adjust="mvt") #in nine domains, forecasting error scores from Tournament 1 predict fprecasting error scores from Tournament 2
model.phase2.predicted.by.phase1.academ.diff<-  lmer(scale((MASE1_w1-MASE1_w2))~scale(log(MASE1_w1))+(1|ResponseId), data=dat_phase2) #effect of inaccuracy in the 12m tournament for changes in inaccuracy between 12 and 6 m tournaments
