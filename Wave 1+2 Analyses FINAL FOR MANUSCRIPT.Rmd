...
sim.w1.rw <- sim.w1 %>% 
  ...
  select(domain, rw.MASE.w1)

sim.w2.rw <- sim.w2 %>% 
  filter(Type == 'Random Walk') %>% 
  mutate(rw.MASE.w2 = response) %>% 
  select(domain, rw.MASE.w2)
...
t1.academ.sorted <- phase1_exp %>%
      ...
      select(team_name, domain, Rank, Nteams, Method.code, Month.1:Month.12,mean_abs_error_w1,MASE1_w1)
...
t1.nonacadem.av.sorted <- phase1 %>% 
  filter(isExpert.factor == 'Prolific') %>%                                                               select(team_name,domain,Month.1:Month.12,mean_abs_error_w1,MASE1_w1,Method.code) %>% 
  group_by(domain) %>% 
...
t1.academ.best.sorted <- phase1 %>% 
  filter(isExpert.factor == 'Academic') %>%                                                               select(team_name,domain,Month.1:Month.12,mean_abs_error_w1,MASE1_w1,Method.code) %>% 
  group_by(domain) %>% 
  summarise(across(where(is.numeric), min)) %>% 
  arrange(domain,MASE1_w1) %>% 
...
t2.academ.sorted <- academic_only %>% 
  ...
  group_by(domain) %>% 
  mutate(Rank = row_number()) %>% 
  add_count(name="Nteams") %>%                                                                            select(team_name,domain,Rank,Nteams,Method.code,phase,revised,Month.7:Month.12,mean_abs_error_w2,MASE1_w2)
...
results<-rbind(t1.academ.sorted, t2.academ.sorted,objective) %>% 
  ungroup() %>% 
  select(-domain,-Method.code, -(phase:revised)) 
...
top.1.MASE.t1 <- phase1 %>% 
  ...
  group_by(domain) %>% 
  dplyr::slice_head(n = 1) %>% select(team_name,mean_abs_error_w1,n_domains,MASE1_w1,Month.1:Month.12,domain,team_size.coded,discipline,previous_tournament.coded,Method.code,model,theory,numpred,parameters,Method.complex,team_expertise) %>%           arrange(MASE1_w1)
...
top.5.MASE.t1 <- phase1 %>% 
  ...
  group_by(team_name) %>%  mutate(n_domains = n()) %>% 
  group_by(domain) %>% 
  dplyr::slice_head(n = 5) %>% select(team_name,MASE1_w1,domain,compare_to_naive_linear_MASE,compare_to_naive_rwf_MASE,team_size.coded,discipline,previous_tournament.coded,Method.code,model,theory,numpred,parameters,Method.complex,team_expertise,n_domains)
...
phase1 %>% 
  ...
  select(team_name,MASE1_w1,domain,team_size.coded,discipline,previous_tournament.coded,Method.code,model,theory,numpred,parameters,Method.complex,team_expertise,team_gender,team_education,team_Age,non_US )%>%
...
top.5.MASE.t2 <- academic_only %>% 
  ...
  arrange(domain,MASE1_w2) %>% 
    group_by(team_name) %>%  mutate(n_domains = n()) %>% 
   group_by(domain) %>% 
  dplyr::slice_head(n = 5) %>% select(team_name,MASE1_w2,domain,n_domains,compare_to_naive_linear_MASE_w2,compare_to_naive_rwf_MASE,team_size.coded,discipline,previous_tournament.coded,Method.code,model,theory,numpred,parameters,Method.complex,team_expertise,phase,revised)
...
academic_only %>% 
  ...
  arrange(domain,MASE1_w2) %>%
  group_by(domain) %>% 
  dplyr::slice_head(n = 10) %>% select(team_name,MASE1_w2,domain,team_size.coded,discipline,previous_tournament.coded,Method.code,model,theory,numpred,parameters,Method.complex,team_expertise) %>%
  ggplot(aes(x = domain, y = team_size.coded)) +
  stat_summary(fun.data="mean_cl_boot",  position=pd)+theme_minimal(base_size = 14) +
  theme(legend.position="bottom") + 
  scale_x_discrete(labels=labels, name="") +
...
academic_only %>% 
  ...
  arrange(domain,MASE1_w2) %>% 
  group_by(domain) %>% 
  dplyr::slice_head(n = 5) %>% select(team_name,MASE1_w1,domain,team_size.coded,discipline,previous_tournament.coded,Method.code,model,theory,numpred,parameters,Method.complex,team_expertise) %>%
  ggplot(aes(x = domain, y = Method.complex)) +
  stat_summary(fun.data="mean_cl_boot",  position=pd) + 
  theme_minimal(base_size = 14) +
  theme(legend.position="bottom") + 
...
academic_only %>% 
  ...
  arrange(domain,MASE1_w2) %>% 
  group_by(domain) %>% 
  dplyr::slice_head(n = 10) %>% select(team_name,MASE1_w2,domain,team_size.coded,discipline,previous_tournament.coded,Method.code,model,theory,numpred,parameters,Method.complex,team_expertise,team_gender,team_education,team_Age,non_US )%>%
  ggplot(aes(x = domain, y = team_gender)) +
  stat_summary(fun.data="mean_cl_boot",  position=pd)+theme_minimal(base_size = 14) +
  theme(legend.position="bottom") + 
  scale_x_discrete(labels=labels, name="")+
...
top.5.MASE%>% select(team_name,MASE,domain,phase,model,revised) %>% group_by(domain) %>% count(team_name)
...
top5.repeats.t1<-top.5.MASE.t1%>% select(team_name,MASE,domain,n_domains) %>% group_by(team_name) %>% count(team_name) %>% arrange(desc(n))
psych::describe(top5.repeats.t1)#14 appear more than once; but M is small = 1.62
top5.repeats.t1.perc<-top5.repeats.t1 %>% left_join(top.5.MASE.t1 %>% select(team_name,n_domains) )
...
top5.repeats.t2<-top.5.MASE.t2%>% select(team_name,MASE,domain,n_domains) %>% group_by(team_name) %>% count(team_name) %>% arrange(desc(n))
psych::describe(top5.repeats.t2)#17 appear more than once; but M is small = 1.67
top5.repeats.t2.perc<-top5.repeats.t2 %>% left_join(top.5.MASE.t2 %>% select(team_name,n_domains) )
...
```{r}
phase1.noexp.dem<-phase1 %>% filter(isExpert.factor=="Prolific") %>% select(ResponseId,Age, Sex, Ethnicity, Education,Residential.Area, Income) %>% dplyr::group_by(ResponseId) %>% 
summarize_all(~ mean(.x, na.rm = TRUE))
psych::describe(phase1.noexp.dem$Age)
prop.table(table(phase1.noexp$Sex)) #46.36% female
prop.table(table(phase1.noexp$Education))
