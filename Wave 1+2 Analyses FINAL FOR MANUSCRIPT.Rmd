...
---
This document reproduces the main analyses, figures, and tables for the Wave 1 + 2 manuscript.

**How to run (from a fresh clone):**

1. In RStudio, open the project.
2. Run `GET_STARTED.R` once (e.g., `source("GET_STARTED.R")`) to install all required packages.
3. Then knit this R Markdown file.

**Key project files (see `GET_STARTED.R` for more details):**

- `GET_STARTED.R` – installs packages and documents project structure.  
- `Wave 1+2 Analyses FINAL FOR MANUSCRIPT.Rmd` – this file; main manuscript analyses.  
- `data/` – raw and derived data (if present).  
- `outputs/` – tables and figures created by this document.

```{r setup, include=FALSE}
# Core tidy + plotting
library(tidyverse)   # ggplot2, dplyr, tidyr, lubridate, etc.
library(ggsci)
library(ggpubr)
library(ggdist)
library(tidyquant)
...
# Global options (match GET_STARTED.R where relevant)
options(
  max.print = 20000,
  scipen = 1000
)
...
knitr::opts_chunk$set(echo = TRUE,
  # --- drawing canvas (inches) -------------
  fig.width  = 12,           # big enough → sharp at 300 dpi
  #fig.asp    = 9/16,         # 16 : 9 aspect ratio
  dpi        = 300,

  # --- how the <img> is shown --------------
  out.width  = "100%",       # occupy the whole column
  out.height = "auto",       # keep the 16 : 9 ratio
  fig.align  = "center"      # still wrap in a centred div
)
```

```{r setup working directory}

setwd("~/GitHub/Forecasting-Tournament") #main working directory
...
```{r actual_change_plot, fig.height = 18}
...
```{r graph_indiv_pred, fig.height = 18}
...
```{r compare scores from tournament 1 - first six months vs. last six months}
...
```{r perc.by.domain.phase1, fig.height = 18}
