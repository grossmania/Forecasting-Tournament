...
author: "Oliver Twardus & Igor Grossmann"
date: "3/15/2022"
...
dat_return <- dat_wave_2$team_name[which(dat_wave_2$team_name %in% initial_participants)]
dat_return <- unique(dat_return)

dat$revised <- ifelse(dat$team_name %in% dat_return, 1, 0)
...
dat_size <- read.csv("team_descriptives_updated.csv", stringsAsFactors = FALSE)
...
dat_counter <- read.csv("Counterfactuals_fixed.csv", stringsAsFactors = FALSE)
dat_counter <- dat_counter[ , c("phase", "team_name", "ResponseId", "domain", "CounterFactual_Presence_Final", "COVID.Final", "Onus.Final", "counterNum")]

dat <- dat %>% left_join(dat_counter, by = c("phase", "ResponseId", "team_name", "domain"))
...
```{r Import revised submission justifications for phase 2}

dat_justification <- read.csv("review_justificationV2.csv", stringsAsFactors = FALSE)
dat_justification <- dat_justification[, c("ResponseId", "team_name", "domain", "justification_dataReceived", "justification_theoreticalInsight", "justification_externalEvent")]
dat_justification$phase <- 2

dat <- dat %>% left_join(dat_justification, by = c("ResponseId", "team_name", "domain", "phase"))

```

```{r coded number of parameters}

dat_param <- read.csv("NumParameters_final.csv", stringsAsFactors = FALSE)

dat_param <- dat_param[, c("ResponseId", "phase", "domain", "parameters_coded")]

dat <- dat %>% left_join(dat_param, by = c("ResponseId", "phase", "domain"))
...
for (i in 1:length(domains)) {
  ...
  trend <- (dat_hist[40, domains[i]] - dat_hist[1, domains[i]]) / 39
...
for (i in 1:length(domains)) {
  ...
  for (n in 1:length(list_val)) {
    if (!is.na(dat$root_mean_sqr_error_w1[list_val[n]]) & dat$root_mean_sqr_error_w1[list_val[n]] < cutoff1) {
      ...
      dat$RMSE_cutoff_Naive_linear[list_val[n]] <- 2
    ...
    if (!is.na(dat$root_mean_sqr_error_w1[list_val[n]]) & dat$root_mean_sqr_error_w1[list_val[n]] < cutoff2) {
      ...
      dat$RMSE_cutoff_Naive_rwf[list_val[n]] <- 2
    }
  }
}

dat$compare_to_naive_linear <- factor(dat$RMSE_cutoff_Naive_linear, levels = c(0, 1, 2),
                          labels = c("Below Naive linear", "Above Naive linear", "Equal to Naive linear"))

dat$compare_to_naive_rwf <- factor(dat$RMSE_cutoff_Naive_rwf, levels = c(0, 1, 2),
                          labels = c("Below Naive rwf", "Above Naive rwf", "Equal to Naive rwf"))

```

```{r Compare forecast MASE to naive approach}


# Compare each prediction's MASE score to the two types of naive approaches generated and create a cutoff score, indicating whether the row's MASE value is greater or less than the naive forecast's MASE value

dat$MASE_cutoff_Naive_linear <- NA
dat$MASE_cutoff_Naive_rwf <- NA


for (i in 1:length(domains)) {
  

  cutoff1 <- dat$MASE1_w1[which(dat$domain == domains[i] & dat$Method %in% "Naive - linear")]
  cutoff2 <- dat$MASE1_w1[which(dat$domain == domains[i] & dat$Method %in% "Naive - rwf")]
  ...
  for (n in 1:length(list_val)) {
    if (!is.na(dat$MASE1_w1[list_val[n]]) & dat$MASE1_w1[list_val[n]] < cutoff1) {
      dat$MASE_cutoff_Naive_linear[list_val[n]] <- 0


    } else if (!is.na(dat$MASE1_w1[list_val[n]]) & dat$MASE1_w1[list_val[n]] > cutoff1){
      dat$MASE_cutoff_Naive_linear[list_val[n]] <- 1

    } else if (!is.na(dat$MASE1_w1[list_val[n]]) & dat$MASE1_w1[list_val[n]] == cutoff1){
      dat$MASE_cutoff_Naive_linear[list_val[n]] <- 2

    }
    
    if (!is.na(dat$MASE1_w1[list_val[n]]) & dat$MASE1_w1[list_val[n]] < cutoff2) {
      dat$MASE_cutoff_Naive_rwf[list_val[n]] <- 0
    } else if (!is.na(dat$MASE1_w1[list_val[n]]) & dat$MASE1_w1[list_val[n]] > cutoff2){
      dat$MASE_cutoff_Naive_rwf[list_val[n]] <- 1
    } else if (!is.na(dat$MASE1_w1[list_val[n]]) & dat$MASE1_w1[list_val[n]] == cutoff2){
      dat$MASE_cutoff_Naive_rwf[list_val[n]] <- 2
    }
  }
}

dat$compare_to_naive_linear_MASE <- factor(dat$MASE_cutoff_Naive_linear, levels = c(0, 1, 2),
                          labels = c("Below Naive linear", "Above Naive linear", "Equal to Naive linear"))

dat$compare_to_naive_rwf_MASE <- factor(dat$MASE_cutoff_Naive_rwf, levels = c(0, 1, 2),
                          labels = c("Below Naive rwf", "Above Naive rwf","Equal to Naive rwf"))
...
###compute mase for phase 1 months 1 to 6

 

```{r compute mase for months 1 to 6 for phase 1 to compare to A. last six months, and to phase 2 }

# create data set that will be merged with dat, that contains additional columns for mean error, root mean square error, mean percent error, mean absolute error, mean absolute percent error and the two types of mean absolute scaled error that are being calculated (#1 uses function declared on line 34, #2 uses Metrics::mase function)
dat_accuracy_w1_firstmonths <- tibble(
  ResponseId_w1_firstmonths = character(),
  domain_w1_firstmonths = character(),
  mean_error_w1_firstmonths = numeric(),
  root_mean_sqr_error_w1_firstmonths = numeric(),
  mean_abs_error_w1_firstmonths = numeric(),
  mean_percent_error_w1_firstmonths = numeric(),
  mean_abs_percent_error_w1_firstmonths = numeric(),
  MASE1_w1_firstmonths = numeric()
)


dat_predicted_w1_firstmonths <- filter(dat_wave_1, Month < 7)
domain_outcomes_w1_firstmonths <- filter(domain_outcomes, Month < 7)
...
for (i in 1:length(unique_teams_predicted_w1)) {
  dat_temp_1_w1_firstmonths <- filter(dat_predicted_w1_firstmonths, ResponseId == unique_teams_predicted_w1[i])
  
  domain_list_w1_firstmonths <- unique(dat_temp_1_w1_firstmonths$domain)
  
  for (n in 1:length(domain_list_w1_firstmonths)) {
    
    dat_temp_2_w1_firstmonths <- filter(dat_temp_1_w1_firstmonths, domain == domain_list_w1_firstmonths[n])
    
    vals_w1_firstmonths <- as.vector(dat_temp_2_w1_firstmonths$value)
    
    objective_data_firstmonths <- dat_hist[ , which(names(dat_hist) == domain_list_w1_firstmonths[n])]
    objective_data_firstmonths <- as.vector((objective_data_firstmonths))

 

    
    outcome_w1_firstmonths <- forecast::accuracy(dat_temp_2_w1_firstmonths$value, domain_outcomes_w1_firstmonths[, which(names(domain_outcomes_w1_firstmonths) == domain_list_w1_firstmonths[n])])

 

    dat_accuracy_w1_firstmonths <- dat_accuracy_w1_firstmonths %>% add_row(
      ResponseId_w1_firstmonths = unique_teams_predicted_w1[i],
      domain_w1_firstmonths = domain_list_w1_firstmonths[n],
      mean_error_w1_firstmonths = outcome_w1_firstmonths[1],
      root_mean_sqr_error_w1_firstmonths = outcome_w1_firstmonths[2],
      mean_abs_error_w1_firstmonths = outcome_w1_firstmonths[3],
      mean_percent_error_w1_firstmonths = outcome_w1_firstmonths[4],
      mean_abs_percent_error_w1_firstmonths = outcome_w1_firstmonths[5],
      MASE1_w1_firstmonths = computeMASE(dat_temp_2_w1_firstmonths$value, objective_data_firstmonths[1:40], objective_data_firstmonths[41:46], 1)
                             )
...
dat_accuracy_w1_firstmonths$phase <- 1

 

# merge dat_accuracy with dat by matching response ID, phase, and domain
dat <- dat %>% left_join(dat_accuracy_w1_firstmonths, by = c("phase" = "phase", "ResponseId" = "ResponseId_w1_firstmonths", "domain" = "domain_w1_firstmonths"))
...
for (i in 1:length(domains)) {
  ...
  trend <- (dat_hist[46, domains[i]] - dat_hist[1, domains[i]]) / 45
  
  objective_data <- domain_outcomes[7:12 , domains[i]]
  objective_data <- as.vector((objective_data))
  
  vals_w2 <- seq(from = start + trend, by = trend, length.out = 6)
  
  naive_acc_w2 <- forecast::accuracy(vals_w2, objective_data)
 ...
 domain = domains[i],
    ...
    MASE1_w2 = computeMASE(vals_w2, dat_hist[1:46, domains[i]], objective_data, 1),
    Method.coded = 6,
    phase = 2
  )
}
...
for (i in 1:length(domains)) {
  ...
  for (n in 1:length(list_val_w2)) {
    if (!is.na(dat$root_mean_sqr_error_w2[list_val_w2[n]]) & dat$root_mean_sqr_error_w2[list_val_w2[n]] < cutoff1_w2) {
      ...
      dat$RMSE_cutoff_Naive_linear_w2[list_val_w2[n]] <- 2
    ...
    if (!is.na(dat$root_mean_sqr_error_w2[list_val_w2[n]]) & dat$root_mean_sqr_error_w2[list_val_w2[n]] < cutoff2_w2) {
      ...
      dat$RMSE_cutoff_Naive_rwf_w2[list_val_w2[n]] <- 2
    }
  }
}


dat$compare_to_naive_linear_w2 <- factor(dat$RMSE_cutoff_Naive_linear_w2, levels = c(0, 1, 2),
                          labels = c("Below Naive linear", "Above Naive linear", "Equal to Naive linear"))

dat$compare_to_naive_rwf_w2 <- factor(dat$RMSE_cutoff_Naive_rwf_w2, levels = c(0, 1, 2),
                          labels = c("Below Naive rwf", "Above Naive rwf", "Equal to Naive rwf"))

```

```{r Compare forecast MASE to naive approach - phase 2}


# Compare each prediction's MASE score to the two types of naive approaches generated and create a cutoff score, indicating whether the row's MASE value is greater or less than the naive forecast's MASE value

dat$MASE_cutoff_Naive_linear_w2 <- NA
dat$MASE_cutoff_Naive_rwf_w2 <- NA


for (i in 1:length(domains)) {
  

  cutoff1 <- dat$MASE1_w2[which(dat$domain == domains[i] & dat$Method %in% "Naive - linear" & dat$phase == 2)]
  cutoff2 <- dat$MASE1_w2[which(dat$domain == domains[i] & dat$Method %in% "Naive - rwf" & dat$phase == 2)]

 
  list_val <- which(dat$domain == domains[i] & !dat$Method %in% c("Naive - rwf", "Naive - linear", "Objective"))

                      
  for (n in 1:length(list_val)) {
    if (!is.na(dat$MASE1_w2[list_val[n]]) & dat$MASE1_w2[list_val[n]] < cutoff1) {
      dat$MASE_cutoff_Naive_linear_w2[list_val[n]] <- 0


    } else if (!is.na(dat$MASE1_w2[list_val[n]]) & dat$MASE1_w2[list_val[n]] > cutoff1){
      dat$MASE_cutoff_Naive_linear_w2[list_val[n]] <- 1


    } else if (!is.na(dat$MASE1_w2[list_val[n]]) & dat$MASE1_w2[list_val[n]] == cutoff1) {
      dat$MASE_cutoff_Naive_linear_w2[list_val[n]] <- 2
    }
    
    if (!is.na(dat$MASE1_w2[list_val[n]]) & dat$MASE1_w2[list_val[n]] < cutoff2) {
      dat$MASE_cutoff_Naive_rwf_w2[list_val[n]] <- 0
      
    } else if (!is.na(dat$MASE1_w2[list_val[n]]) & dat$MASE1_w2[list_val[n]] > cutoff2){
      dat$MASE_cutoff_Naive_rwf_w2[list_val[n]] <- 1
      
    } else if (!is.na(dat$MASE1_w2[list_val[n]]) & dat$MASE1_w2[list_val[n]] == cutoff2) {
      dat$MASE_cutoff_Naive_rwf_w2[list_val[n]] <- 2
    }
  }
}

dat$compare_to_naive_linear_MASE_w2 <- factor(dat$MASE_cutoff_Naive_linear_w2, levels = c(0, 1, 2),
                          labels = c("Below Naive linear", "Above Naive linear", "Equal to Naive linear"))

dat$compare_to_naive_rwf_MASE_w2 <- factor(dat$MASE_cutoff_Naive_rwf_w2, levels = c(0, 1, 2),
                          labels = c("Below Naive rwf", "Above Naive rwf", "Equal to Naive rwf"))
...
dat_final <- dat[, c("Duration..in.seconds.", "ResponseId", "phase", "team_name", "domain", "Month.1", "Month.2", "Month.3",
                     "Month.4", "Month.5", "Month.6", "Month.7", "Month.8", "Month.9", "Month.10", "Month.11", "Month.12",
                     "Month.13", "Month.14", "Month.15", "Month.16", "Month.17", "Month.18", "confidence", "subexpert", "pub",
                     "basis", "basis_5_TEXT", "model", "theory", "parameters" , "numpred", "covidcondyn", "covidmeas",
                     "covidmeas_6_TEXT", "coviddesc", "covidest_1", "covidest_2", "covidest_3", "covidest_4", "covidest_5",
                     "covidest_6", "covidest_7", "covidest_8", "covidest_9", "covidest_10", "covidest_11", "covidest_12",
                     "datatrain", "counterfact", "counter_imp", "counter_yn", "othercounter", "othercountim", "Method", "isExpert",
                     "ResponseId.old", "revised", "team_size", "team_expertise", "team_size.coded", "team_discipline.coded",
                     "team_discipline.missingdat", "previous_tournament.coded", "previous_tournament.missingdat", "Age", "Sex",
                     "Genderident", "Genderident_5_TEXT",
                     "Occupation", "Ethnicity", "Education", "Religion", "Religion_Other", "Politics_1", "Country.of.Residence",
                     "Residential.Area", "Income", "time_upload", "Method.coded", "Method.complex", "DataOnly", "mean_error_w1",
                     "root_mean_sqr_error_w1", "mean_abs_error_w1", "mean_percent_error_w1", "mean_abs_percent_error_w1", "MASE1_w1",
                     "RMSE_cutoff_Naive_linear", "RMSE_cutoff_Naive_rwf", "compare_to_naive_linear", "compare_to_naive_rwf",
                     "MASE_cutoff_Naive_linear", "MASE_cutoff_Naive_rwf", "compare_to_naive_linear_MASE",
                     "compare_to_naive_rwf_MASE", "time_spent",  "mean_error_covid",  "root_mean_sqr_error_covid",
                     "mean_abs_error_covid", "mean_percent_error_covid", "mean_abs_percent_error_covid", "MASE1_covid",
                     "mean_error_w1_firstmonths", "root_mean_sqr_error_w1_firstmonths", "mean_abs_error_w1_firstmonths",
                     "mean_percent_error_w1_firstmonths", "mean_abs_percent_error_w1_firstmonths", "MASE1_w1_firstmonths",
                     "mean_error_w1_lastmonths", "root_mean_sqr_error_w1_lastmonths", "mean_abs_error_w1_lastmonths",
                     "mean_percent_error_w1_lastmonths", "mean_abs_percent_error_w1_lastmonths", "MASE1_w1_lastmonths",
                     "mean_error_w2", "root_mean_sqr_error_w2", "mean_abs_error_w2", "mean_percent_error_w2",
                     "mean_abs_percent_error_w2", "MASE1_w2", "RMSE_cutoff_Naive_linear_w2", "RMSE_cutoff_Naive_rwf_w2",
                     "compare_to_naive_linear_w2", "compare_to_naive_rwf_w2", "MASE_cutoff_Naive_linear_w2",
                     "MASE_cutoff_Naive_rwf_w2", "compare_to_naive_linear_MASE_w2", "compare_to_naive_rwf_MASE_w2", "team_gender",
                     "team_education", "team_Age", "non_US", "CounterFactual_Presence_Final", "COVID.Final", "Onus.Final",
                     "counterNum", "justification_dataReceived", "justification_theoreticalInsight", "justification_externalEvent",
                     "parameters_coded" 
...
dat_final$TournamentStart[dat$phase == 1 | (dat$phase == 2 & dat$revised == 1)] <- "May"
dat_final$TournamentStart[dat$phase == 2 & dat$revised == 0] <- "November"

# Add column indicating whether the forecast is a new one or an updated one
dat_final$ForecastisUpdated[dat$phase == 2 & dat$revised == 1] <- "Updated in Nov"
dat_final$ForecastisUpdated[dat$phase == 2 & dat$revised == 0] <- "Novel Forecast in November"
...
write.csv(dat_final, "Wave1+2data.csv")
