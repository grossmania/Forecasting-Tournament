...
dat_coded[which(dat_coded$team_name == "PatrÃcia Arriaga"), "team_name"] <- "Patricia Arriaga"
...
for (i in 1:length(unique_teams_predicted_w1)) {
  ...
  for (n in 1:length(domain_list_w1)) {
    ...
    dat_accuracy_w1 <- dat_accuracy_w1 %>% add_row(
      ...
      MASE2_w1 = Metrics::mase(objective_data[41:52], vals_w1, step_size = 1)
                             )
...
for (i in 1:length(domains)) {
  ...
  dat <- dat %>% add_row(
    ...
    MASE2_w1 = Metrics::mase(objective_data, vals_w1, step_size = 1),
    Method.coded = 6,
    phase = 1
  )
}
...
for (i in 1:length(domains)) {
  ...
  dat <- dat %>% add_row(
    ...
    MASE2_w1 = Metrics::mase(objective_data, naive_forecast.test, step_size = 1),
...
for (i in rows_ideol) {
  ...
  if ((dat$Method.coded[i] != 5 | is.na(dat$Method.coded[i])) & dat$phase[i] == 1) {
    ...
    dat$MASE2_w1[i] = Metrics::mase(hist_w1, val_w1, step_size = 1) 
  }
...
for (i in 1:length(unique_teams_predicted_covid_death)) {
  ...
  for (n in 1:length(domain_list_covid)) {
    ...
    dat_accuracy_covid <- dat_accuracy_covid %>% add_row(
      ResponseId = unique_teams_predicted_w1[i],
      domain = domain_list_w1[n],
      mean_error_covid = outcome_covid[1],
      root_mean_sqr_error_covid = outcome_w1[2],
      mean_abs_error_covid = outcome_w1[3],
      mean_percent_error_covid = outcome_w1[4],
      mean_abs_percent_error_covid = outcome_w1[5],
      MASE1_covid = computeMASE(dat_temp_2_covid$value, objective_data[37:40], objective_data[41:52], 1),
      MASE2_covid = Metrics::mase(objective_data[41:52], vals_covid, step_size = 1)
                             )
...
for (i in 1:length(unique_teams_predicted_w1)) {
  ...
  for (n in 1:length(domain_list_w1_lastmonths)) {
    ...
    dat_accuracy_w1_lastmonths <- dat_accuracy_w1_lastmonths %>% add_row(
      ...
      MASE2_w1_lastmonths = Metrics::mase(objective_data_lastmonths[47:52], vals_w1_lastmonths, step_size = 1)
                             )
...
for (i in rows_ideol) {
  ...
  if (dat$Method.coded[i] != 5 | is.na(dat$Method.coded[i])) {
    ...
    dat$MASE2_w1_lastmonths[i] = Metrics::mase(hist_w1_lastmonths, val_w1_lastmonths , step_size = 1) 
  }
...
for (i in 1:length(unique_teams_predicted_w2)) {
  ...
  for (n in 1:length(domain_list_w2)) {
    ...
    dat_accuracy_w2 <- dat_accuracy_w2 %>% add_row(
      ...
      MASE2_w2 = Metrics::mase(objective_data[47:52], vals_w2, step_size = 1)
                             )

  }
}

dat_accuracy_w2$phase <- 2
...
for (i in 1:length(domains)) {
 ...
 domain = domains[i],
    ...
    MASE2_w2 = Metrics::mase(objective_data, vals_w2, step_size = 1),
    Method.coded = 6,
    phase = 2
  )
}
...
for (i in 1:length(domains)) {
  ...
  naive_forecast.tib_w2 <- as_tibble(naive_forecast)
  ...
  dat <- dat %>% add_row(
    ...
    domain = domains[i],
    Method = "Naive - rwf",
    Month.7 = naive_forecast.test_w2[7],
    Month.8 = naive_forecast.test_w2[8],
    Month.9 = naive_forecast.test_w2[9],
    Month.10 = naive_forecast.test_w2[10],
    Month.11 = naive_forecast.test_w2[11],
    Month.12 = naive_forecast.test_w2[12],
    ...
    MASE2_w2 = Metrics::mase(objective_data, naive_forecast.test_w2, step_size = 1),
    #mean_abs_scaled_error = pull(naive_acc.tib[2, 6]),
    Method.coded = 7,
    phase = 2
  )
...
# get list of relevant rows that submitted predictions for ideology domains
rows_ideol <- which((dat$domain == "ideoldem" | dat$domain == "ideolrep") & dat$phase == 2)
...
for (i in rows_ideol) {
  ...
  if (dat$Method.coded[i] != 5 | is.na(dat$Method.coded[i])) {
    ...
    dat$MASE2_w2[i] = Metrics::mase(hist_w2, val_w2 , step_size = 1) 
  }
...
for (i in 1:length(domains)) {
  ...
  hist <- dat[which(dat$domain == domains[i] & dat$Method == "Objective" & dat$phase == 2), ]
...
write.csv(dat, "Wave1+2data_2021-07-21.csv")
write.csv(demographics_unique, "Wave1+2demographics_2021-07-21.csv")
...
write.csv(dat_comp_1, "team_size_2021-07-21.csv")
```
