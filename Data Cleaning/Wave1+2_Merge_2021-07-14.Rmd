...
dat_coded <- dat_coded[ , c("ResponseId", "phase", "Issue", "Type.Final", "Complexity.Final", "DataOnly")]
...
for (i in 1:length(unique_teams1)) {
  ...
  dat$ResponseId[which(dat$team_name == unique_teams1[i]& dat$phase == 1)] <- dat$ResponseId[row_list1[1]]
...
for (i in 1:length(unique_teams2)) {
  ...
  dat$ResponseId[which(dat$team_name == unique_teams[i]& dat$phase == 2)] <- dat$ResponseId[row_list2[1]]
...
dat <- dat[, c(7, 10, 21, 27, 29:49, 55:58, 60:71, 77:82, 92, 100:102, 104:112, 122, 127:128, 130, 132:138, 170:179, 181:202)]
...
dat$ForecastisUpdated <- ifelse(dat$phase == 2 & dat$ revised == 1, 1, 0)

```



```{r Create Long data set with difference scores, eval=FALSE, include=FALSE}
