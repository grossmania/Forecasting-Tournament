...
for (i in 1:length(unique_teams_predicted_w1)) {
  ...
  for (n in 1:length(domain_list_w1)) {
    ...
    dat_accuracy_w1 <- dat_accuracy_w1 %>% add_row(
      ...
      MASE2_w1 = mase(objective_data[41:52], vals_w1, step_size = 1)
                             )
...
for (i in 1:length(domains)) {
  ...
  dat <- dat %>% add_row(
    ...
    MASE2_w1 = mase(objective_data, vals_w1, step_size = 1),
    Method.coded = 6,
    phase = 1
  )
}
...
for (i in 1:length(domains)) {
  ...
  dat <- dat %>% add_row(
    ...
    MASE2_w1 = mase(objective_data, naive_forecast.test, step_size = 1),
...
for (i in rows_ideol) {
  ...
  if ((dat$Method.coded[i] != 5 | is.na(dat$Method.coded[i])) & dat$phase[i] == 1) {
    ...
    dat$MASE2_w1[i] = mase(hist_w1, val_w1, step_size = 1) 
  }
...
for (i in 1:length(unique_teams_predicted_w2)) {
  ...
  for (n in 1:length(domain_list_w2)) {
    ...
    dat_accuracy_w2 <- dat_accuracy_w2 %>% add_row(
      ...
      MASE2_w2 = mase(objective_data[47:52], vals_w2, step_size = 1)
                             )
...
for (i in 1:length(domains)) {
 ...
 domain = domains[i],
    ...
    MASE2_w2 = mase(objective_data, vals_w2, step_size = 1),
    Method.coded = 6,
    phase = 2
  )
}
...
for (i in 1:length(domains)) {
  ...
  dat <- dat %>% add_row(
    ...
    MASE2_w2 = mase(objective_data, naive_forecast.test_w2, step_size = 1),
    #mean_abs_scaled_error = pull(naive_acc.tib[2, 6]),
    Method.coded = 7,
    phase = 2
  )
...
for (i in rows_ideol) {
  ...
  if (dat$Method.coded[i] != 5 | is.na(dat$Method.coded[i])) {
    ...
    dat$MASE2_w2[i] = mase(hist_w2, val_w2 , step_size = 1) 
  }
