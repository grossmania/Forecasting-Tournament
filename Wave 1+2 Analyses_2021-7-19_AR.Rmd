...
emmeans(model.long.phase1,pairwise ~Method.code*Month0|domain, adjust = "none") #overall month (half a year estimate)
...
emmeans(model.phase1,pairwise ~domain|Method.code, adjust = "none")$contrast
...
#sig contrast difference among academics for #explicit asian bias, implicit asian bias, ideology  - democrats, ideology-republicans, life satisfaction
#marginal trend for contrast difference among academics for #explicit afrian american bias, implicit gender bias


#visualize
dat %>% subset(phase == 1 & !is.na(isExpert.factor)& !is.na(revised)&Method.code!="Ground Truth"& Method.code!="Naive-linear"&Method.code!="Naive-rfw") %>% 
 ggplot(aes(x = domain, y = MASE1_w1, colour = updated , fill=updated ))+
 stat_summary(fun.data="mean_cl_boot",  position=pd)+  theme_minimal(base_size = 14) +geom_hline(yintercept =1, linetype='dashed', color='red', 14)+
theme(legend.position="bottom") +scale_color_d3()+scale_fill_d3()+  
  labs(colour = "",fill="", x="",y="MASE (M +/- 95%CI)") 

## Examine effects of team type and confidence (instead of forecasting type) as predictors.

### overall MASE - NOTE ACADEMICS NOT INCLUDED!!!
#### first, confidence
...
emtrends(model.phase1.conf,specs=pairwise~domain,var="confidence") #confidence does not play a role for any of the domains, and when comparing domains to each other
...
car::Anova(model.phase1.team,type="III") #marginal interaction between domain and team size
summ(model.phase1.team, digits=4, center=T)
emtrends(model.phase1.team,specs=pairwise~domain,var="team_size") #confidence does not play a role for any of the domains, and when comparing domains to each other
...
### Time-specific - NOTE ACADEMICS NOT INCLUDED!!!

#### first, confidence
model.long.phase1.conf<-  lmer(value.dif~domain*confidence*Month0+(1|ResponseId), data=dat_long_phase1)
car::Anova(model.long.phase1.conf,type="III") #no interaction, but a main effect of confidence, and domain by confidence interaction. #note need to check with academics!
summ(model.long.phase1.conf, digits=4, center=T) #the more confidence, the greater the error!
emtrends(model.long.phase1.conf,specs=pairwise~domain*Month0,var="confidence") #confidence does  play a role for explicit bias (african and asian american), implicit african american bias. But not for other domains.
...
emmeans(model.long.phase1.team3,pairwise ~teamS|domain*Month0, adjust = "none") #nimplicit and explicit asian bias - larger team is less inaaccurate (but note reversal for explicit african american bias.)
...
emmeans(model.long.phase1.multidis.team,pairwise ~is_multidisciplinary|domain*Month0, adjust = "none") #for explicit asian, more interdiscplinary is better (less bias)
...
car::Anova(model.phase1.conf,type="III") #
summ(model.phase1.conf, digits=4, center=T)
emtrends(model.phase1.conf,specs=pairwise~domain,var="confidence") #
