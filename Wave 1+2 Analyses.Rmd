...
hist_var<-as_tibble(historical_tsbl) %>% pivot_longer(negaffect:polar,names_to="Domain",values_to="Score") %>%
  filter(Month < 0)%>%group_by(Domain) %>% select(Domain,Score) %>% mutate(sd_hist = sd(Score), mad_hist = mad(Score))

hist_all_summary<-hist_var %>% select(Domain,sd_hist,mad_hist) %>% group_by(Domain) %>% summarise_all(mean) %>% 
  rename( domain = Domain) %>% arrange(sd_hist) #got SD and MAD scores for capturing variability in historical data
...
phase1_exp<-phase1_exp %>% left_join(hist_all_summary)
...
model.phase1<-  lmer(log(MASE1_w1)~domain*Method.code+(1|ResponseId), data=phase1)
...
#  labs(colour = "Sample",fill="Sample", x="",y="MASE (M +/- 95%CI)") #not very useful graph

##try to estimate overall fixed effects across domains

model.phase1.total<-  lmer(log(MASE1_w1)~Method.code+(1|ResponseId), data=phase1)
car::Anova(model.phase1.total,type="III") #overall, a main effect of method
summ(model.phase1.total, digits=4)
emmeans(model.phase1.total,trt.vs.ctrl ~Method.code, adjust = "none") #data driven sig different from lay people, but not for intuition vs. lay people.
...
model.phase1.contrast<-  lmer(log(MASE1_w1)~domain*method.contrast+(1|ResponseId), data=phase1_exp)
...
model.phase1.SD<-  lmer(log(MASE1_w1)~sd_hist +(1|ResponseId), data=phase1_exp)
car::Anova(model.phase1.SD,type="III") ##variability in historical data did not matter

summary(lm(log(MASE1_w1)~mad_hist, data=phase1_exp))

model.phase1.MAD<-  lmer(log(MASE1_w1)~mad_hist +(1|ResponseId), data=phase1_exp)
...
#add historical variability data (as extra variable)
dat_phase2<-dat_phase2 %>% left_join(hist_all_summary)
...
#For models evaluating overall accuracy of the forecasted model, we will use forecasting type (purely theoretical, purely data-driven and hybrid models), forecasting domain as predictors, with MASE scores nested within teams. 
model.phase2<-  lmer(log(MASE1_w2)~domain*Method.code+(1|ResponseId), data=academic_only)
...
##across all domains
model.phase2.total<-  lmer(log(MASE1_w2)~Method.code+(1|ResponseId), data=academic_only)
...
summary(lm(log(MASE1_w2)~sd_hist, data=dat_phase2))
model.phase2.SD<-  lmer(log(MASE1_w1)~sd_hist +(1|ResponseId), data=dat_phase2)
car::Anova(model.phase2.SD,type="III") ##variability in historical data did not matter

summary(lm(log(MASE1_w2)~mad_hist, data=dat_phase2))

model.phase2.MAD<-  lmer(log(MASE1_w2)~mad_hist +(1|ResponseId), data=dat_phase2)
car::Anova(model.phase2.MAD,type="III") ##variability in historical data did not matter

#### Fourteen: number of domains the team made predictions about

model.phase2.Ndomains<-  lmer(log(MASE1_w2)~domain*n_domains+(1|ResponseId), data=dat_phase2)
car::Anova(model.phase2.Ndomains,type="III") #marginal effect of number of domains and a sig interaction
