...
car::Anova(model.phase1.team,type="III") #marg interaction between domain and team size
...
model.phase1.counterfac<-  lmer(MASE1_w1~domain*Presence.Final+(1|ResponseId), data=phase1_exp)
car::Anova(model.phase1.counterfac,type="III") #interdisciplinary did not matter
summ(model.phase1.counterfac, digits=4, center=T)
emmeans(model.phase1.counterfac,pairwise ~Presence.Final|domain, adjust = "none") #nonsig
...
#####just count
model.long.phase1.team<-  lmer(value.dif~domain*team_size.coded*Month0+(1|ResponseId), data=dat_long_phase1)
...
emmeans(model.long.phase1.team3, pairwise~teamS|domain*Month0, adjust = "none",at=list(Month0=c(0,5,11))) #implicit and explicit asian bias - larger team is less inaccurate (but note reversal for explicit african american bias.)
