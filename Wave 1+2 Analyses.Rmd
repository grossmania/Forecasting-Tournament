...
#### fourth, Rating of 1-7 of whether participants considered themselves experts on the domain being predicted
model.long.phase1.subexpert<-  lmer(value.dif~domain*subexpert*Month0+(1|ResponseId), data=dat_long_phase1)
...
model.loong.phase1.obexpert<-  lmer(value.dif~domain*objectivexpert*Month0+(1|ResponseId), data=dat_long_phase1)
...
model.long.phase1.predictors<-  lmer(value.dif~numpred*domain*Month0+(1|ResponseId), data=dat_long_phase1)
...
#### seventh, complexity of the model
model.longphase1.complex<-  lmer(value.dif~domain*Method.complex*Month0+(1|ResponseId), data=dat_long_phase1)
...
#### Eight, counterfactuals (yes/no) when reflecting on predictions
model.long.phase1.counterfac<-  lmer(value.dif~domain*CounterFactual_Presence.Final*Month0+(1|ResponseId), data=dat_long_phase1)
...
model.phase1.wcovid<-  lmer(MASE1_w1~domain*as.factor(covidcondyn)+(1|ResponseId), data=phase1_exp) 
...
model.phase1.long.wcovid<-  lmer(value.dif~Month0*as.factor(covidcondyn)*domain+(1|ResponseId), data=subset(dat_long_phase1, isExpert.factor == 'Academic')) #
...
model.phase1.long.covid<-  lmer(value.dif~Month0*MASE1_covid*domain+(1|ResponseId), data=subset(dat_long_phase1, isExpert.factor == 'Academic')) #
...
dat_long_phase2<-dat_long %>%subset(phase == 1 & !is.na(Method.code)& Method.code!="Ground Truth"& Method.code!="Naive-linear"&Method.code!="Naive-rfw")
dat_long_phase2$Method.code <- relevel(factor(dat_long_phase2$Method.code), "Lay People") #use lay people as a reference group
dat_long_phase2$Month0<-dat_long_phase2$Month-1
...
model.long.phase2<-  lmer(value.dif~domain*Method.code*Month0+(1|ResponseId), data=dat_long_phase2)
...
emmeans(model.long.phase2,specs = trt.vs.ctrl ~Method.code|domain|Month0, adjust = "none", at=list(Month0=c(0,5,11))) #overall month (half a year estimate), contrast to lay people. Lay ppl sig worse for eafric
emmeans(model.long.phase2,pairwise ~domain|Method.code, adjust = "none") #overall differences by domain.

#get scores for analyses
model.long.phase2<-  lmer(value.dif~domain*Method.code*as.factor(Month0)+(1|ResponseId), data=dat_long_phase2)
car::Anova(model.long.phase2,type="III") #sig effect of domain, method, Month, and domain * method

data.long.phase2.abs.dev<-as.data.frame(emmeans(model.long.phase2,pairwise ~Method.code|domain|as.factor(Month0), adjust = "none")$emmeans) #overall month (half a year estimate)
...
#analyses of phase 1  - MASE overall
#For models evaluating overall accuracy of the forecasted model, we will use forecasting type (purely theoretical, purely data-driven and hybrid models), forecasting domain as predictors, with MASE scores nested within teams. 
model.phase2<-  lmer(MASE1_w1~domain*Method.code+(1|ResponseId), data=phase2)
...
model.phase2.contrast<-  lmer(MASE1_w1~domain*method.contrast+(1|ResponseId), data=phase2_exp)
...
model.phase2.update<-  lmer(MASE1_w1~domain*updated+(1|ResponseId), data=phase2_exp)
...
### overall MASE 
#### first, confidence
model.phase2.conf<-  lmer(MASE1_w1~domain*confidence+(1|ResponseId), data=phase2_exp)
...
#####just count
model.phase2.team<-  lmer(MASE1_w1~domain*team_size+(1|ResponseId), data=phase2_exp)
...
#####apriori defined groups
model.phase2.team3<-  lmer(MASE1_w1~domain*teamS+(1|ResponseId), data=phase2_exp)
...
#### third, multidisciplinarity of the teams - just academics
model.phase2.multidis.team<-  lmer(MASE1_w1~domain*is_multidisciplinary+(1|ResponseId), data=phase2_exp)
...
#### fourth, Rating of 1-7 of whether participants considered themselves experts on the domain being predicted
model.phase2.subexpert<-  lmer(MASE1_w1~domain*subexpert+(1|ResponseId), data=phase2_exp)
...
model.phase2.obexpert<-  lmer(MASE1_w1~domain*objectivexpert+(1|ResponseId), data=phase2_exp)
...
#### six, number of predictors in the model 
model.phase2.predictors<-  lmer(MASE1_w1~numpred*domain+(1|ResponseId), data=phase2_exp)
...
#### seventh, complexity of the model
model.phase2.complex<-  lmer(MASE1_w1~domain*Method.complex+(1|ResponseId), data=phase2_exp)
...
#### Eight, counterfactuals (yes/no) when reflecting on predictions
model.phase2.counterfac<-  lmer(MASE1_w1~domain*CounterFactual_Presence.Final+(1|ResponseId), data=phase2_exp)
...
#### first, confidence
model.long.phase2.conf<-  lmer(value.dif~domain*confidence*Month0+(1|ResponseId), data=subset(dat_long_phase2, isExpert.factor == 'Academic'))
...
#####just count
model.long.phase2.team<-  lmer(value.dif~domain*team_size*Month0+(1|ResponseId), data=dat_long_phase2)
...
#####apriori defined groups
model.long.phase2.team3<-  lmer(value.dif~domain*teamS*Month0+(1|ResponseId), data=dat_long_phase2)
...
#### third, multidisciplinarity of the teams - just academics
model.long.phase2.multidis.team<-  lmer(value.dif~domain*is_multidisciplinary*Month0+(1|ResponseId), data=dat_long_phase2)
...
#### fourth, Rating of 1-7 of whether participants considered themselves experts on the domain being predicted
model.long.phase2.subexpert<-  lmer(value.dif~domain*subexpert*Month0+(1|ResponseId), data=dat_long_phase2)
...
model.loong.phase2.obexpert<-  lmer(value.dif~domain*objectivexpert*Month0+(1|ResponseId), data=dat_long_phase2)
...
#### six, number of predictors in the model NEED TO BE CORRECTED!
model.long.phase2.predictors<-  lmer(value.dif~numpred*domain*Month0+(1|ResponseId), data=dat_long_phase2)
...
#### seventh, complexity of the model
model.longphase2.complex<-  lmer(value.dif~domain*Method.complex*Month0+(1|ResponseId), data=dat_long_phase2)
...
#### Eight, counterfactuals (yes/no) when reflecting on predictions
model.long.phase2.counterfac<-  lmer(value.dif~domain*CounterFactual_Presence.Final*Month0+(1|ResponseId), data=dat_long_phase2)
...
model.phase2.wcovid<-  lmer(MASE1_w1~domain*as.factor(covidcondyn)+(1|ResponseId), data=phase2_exp) 
...
model.phase2.long.wcovid<-  lmer(value.dif~Month0*as.factor(covidcondyn)*domain+(1|ResponseId), data=subset(dat_long_phase2, isExpert.factor == 'Academic')) #
...
#### first, MASE
model.phase2.covid<-  lmer(MASE1_w1~domain*MASE1_covid+(1|ResponseId), data=phase2_exp) 
...
#### next, absolute perc error
model.phase2.long.covid<-  lmer(value.dif~Month0*MASE1_covid*domain+(1|ResponseId), data=subset(dat_long_phase2, isExpert.factor == 'Academic')) #
