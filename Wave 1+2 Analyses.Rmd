...
model.long.phase1<-  lmer(value.dif~domain*Method.code*as.factor(Month0)+(1|domain/ResponseId), data=dat_long_phase1)
car::Anova(model.long.phase1,type="III") #sig effect of domain, method, Month, and domain * method
...
model.phase1<-  lmer(MASE1_w1~domain*Method.code+(1|domain/ResponseId), data=phase1)
...
model.phase1.contrast<-  lmer(MASE1_w1~domain*method.contrast+(1|domain/ResponseId), data=phase1_exp)
...
model.phase1.update<-  lmer(MASE1_w1~domain*updated+(1|domain/ResponseId), data=phase1_exp)
...
### overall MASE 
#### first, confidence
model.phase1.conf<-  lmer(MASE1_w1~domain*confidence+(1|domain/ResponseId), data=phase1_exp)
...
#####just count
model.phase1.team<-  lmer(MASE1_w1~domain*team_size+(1|domain/ResponseId), data=phase1_exp)
...
emtrends(model.phase1.team,specs=pairwise~domain,var="team_size") #nothing
#####apriori defined groups
model.phase1.team3<-  lmer(MASE1_w1~domain*teamS+(1|domain/ResponseId), data=phase1_exp)
...
model.phase1.multidis.team<-  lmer(MASE1_w1~domain*is_multidisciplinary+(1|domain/ResponseId), data=phase1_exp)
...
model.phase1.subexpert<-  lmer(MASE1_w1~domain*subexpert+(1|domain/ResponseId), data=phase1_exp)
...
model.phase1.obexpert<-  lmer(MASE1_w1~domain*objectivexpert+(1|domain/ResponseId), data=phase1_exp)
...
model.phase1.predictors<-  lmer(MASE1_w1~numpred*domain+(1|domain/ResponseId), data=phase1_exp)
...
model.phase1.complex<-  lmer(MASE1_w1~domain*Method.complex+(1|domain/ResponseId), data=phase1_exp)
...
model.phase1.counterfac<-  lmer(MASE1_w1~domain*CounterFactual_Presence.Final+(1|domain/ResponseId), data=phase1_exp)
...
#####just count
model.long.phase1.team<-  lmer(value.dif~domain*team_size*Month0+(1|domain/ResponseId), data=dat_long_phase1)
...
car::Anova(model.long.phase1.team3,type="III") #marg interaction between domain and team size, and no main effect of team size
summ(model.long.phase1.team3, digits=4, center=T) #
emmeans(model.long.phase1.team3, pairwise~teamS|domain, adjust = "none") #explicit asian bias - larger team is less inaaccurate 
...
car::Anova(model.long.phase1.subexpert,type="III") #
summ(model.long.phase1.subexpert, digits=4, center=T)
emtrends(model.long.phase1.subexpert,specs=pairwise~domain,var="subexpert") #

#### fifth, objective expertise based on publications in the domain (yes - no)
dat_long_phase1$objectivexpert<-ifelse(dat_long_phase1$pub==1,"Expert",ifelse(dat_long_phase1$pub==2,"Non Expert",NA))
model.loong.phase1.obexpert<-  lmer(value.dif~domain*objectivexpert*Month0+(1|domain/ResponseId), data=dat_long_phase1)
car::Anova(model.loong.phase1.obexpert,type="III") #pubs play a role!
summ(model.loong.phase1.obexpert, digits=4, center=T)
emmeans(model.loong.phase1.obexpert,pairwise ~objectivexpert|domain, adjust = "none") #nonsig
...
car::Anova(model.long.phase1.predictors,type="III") #npred matters!
...
#### seventh, complexity of the model
model.longphase1.complex<-  lmer(value.dif~domain*Method.complex*Month0+(1|domain/ResponseId), data=dat_long_phase1)
car::Anova(model.longphase1.complex,type="III") #complexity matters
summ(model.longphase1.complex, digits=4, center=T)
emtrends(model.longphase1.complex,specs=~domain,var="Method.complex") #
sjPlot::plot_model(model.longphase1.complex,type="int")
...
model.phase1.wcovid<-  lmer(MASE1_w1~domain*as.factor(covidcondyn)+(1|domain/ResponseId), data=phase1_exp) 
car::Anova(model.phase1.wcovid,type="III") #sig interaction
summ(model.phase1.wcovid, digits=4, center=T)
emmeans(model.phase1.wcovid,pairwise ~as.factor(covidcondyn)|domain, adjust = "none")
 
model.phase1.long.wcovid<-  lmer(value.dif~Month0*as.factor(covidcondyn)*domain+(1|domain/ResponseId), data=subset(dat_long_phase1, isExpert.factor == 'Academic')) #
...
#### first, MASE
model.phase1.covid<-  lmer(MASE1_w1~domain*MASE1_covid+(1|domain/ResponseId), data=phase1_exp) 
...
#######JUST PLOT _ ==>ELEGANT BUT ERRORS! IT MISSES THE WITHIN-PERSON STRUCTURE AND REPEATED OBSERVATIONS
#dat_long_phase2 %>% ggplot(aes(x = as.factor(Month), y = value.dif,colour = Method.code, fill=Method.code))+
#   stat_summary(fun.data="mean_cl_boot",  position=pd)+
#    facet_wrap(vars(domain), scales = "free", nrow = 4)+theme_minimal(base_size = 14) +
#   theme(legend.position="bottom") +scale_color_d3()+scale_fill_d3()+
#  labs(colour = "Sample",fill="Sample", x="Time (in months)",y="Absolute Percentage Deviation (M +/- 95%CI)")
...
car::Anova(model.long.phase2,type="III") #sig effect of domain, marg for method, Month, and domain * method * month
...
emmeans(model.long.phase2,specs = trt.vs.ctrl ~Method.code|domain|Month0, adjust = "none", at=list(Month0=c(0,1,2,3,4,5))) #overall month (half a year estimate), contrast to lay people. Lay ppl sig worse for eafric
...
#get scores for analyses
model.long.phase2<-  lmer(value.dif~domain*Method.code*as.factor(Month)+(1|domain/ResponseId), data=dat_long_phase2)
car::Anova(model.long.phase2,type="III") #sig effect of domain, method, Month, and domain * method

data.long.phase2.abs.dev<-as.data.frame(emmeans(model.long.phase2,pairwise ~Method.code*domain*as.factor(Month), adjust = "none",  type = "response")$emmeans) #overall month (half a year estimate)
...
phase2$Method.code <- relevel(factor(phase2$Method.code), "Lay People") #use lay people as a reference group

#analyses of phase 1  - MASE overall
#For models evaluating overall accuracy of the forecasted model, we will use forecasting type (purely theoretical, purely data-driven and hybrid models), forecasting domain as predictors, with MASE scores nested within teams. 
model.phase2<-  lmer(MASE1_w1~domain*Method.code+(1|domain/ResponseId), data=phase2)
car::Anova(model.phase2,type="III") #no interaction, so just look at main effects

summ(model.phase2, digits=4)
emmeans(model.phase2,pairwise ~Method.code|domain, adjust = "none")
emmeans(model.phase2,trt.vs.ctrl ~Method.code|domain, adjust = "none") #lay vs. rest
...
model.phase2.contrast<-  lmer(MASE1_w1~domain*method.contrast+(1|domain/ResponseId), data=phase2_exp)
...
## EXAMINE EFFECTS OF UPDATING FOR PHASE I PREDICTIONS AMONG ACADEMICS
phase2_exp$updated<-ifelse(phase2_exp$revised==1,"update","no update")

model.phase2.update<-  lmer(MASE1_w1~domain*updated+(1|domain/ResponseId), data=phase2_exp)
...
##### second, team type - just academics
phase2_exp$teamS<-as.factor(ifelse(phase2_exp$team_size>=6,3,ifelse(phase2_exp$team_size<6&phase2_exp$team_size>1,2,ifelse(phase2_exp$team_size==1,1,NA))))

#####just count
model.phase2.team<-  lmer(MASE1_w1~domain*team_size+(1|domain/ResponseId), data=phase2_exp)
...
emtrends(model.phase2.team,specs=pairwise~domain,var="team_size") #nothing
#####apriori defined groups
...
##### second, team type - just academics
dat_long_phase2$teamS<-as.factor(ifelse(dat_long_phase2$team_size>=6,3,ifelse(dat_long_phase2$team_size<6&dat_long_phase2$team_size>1,2,ifelse(dat_long_phase2$team_size==1,1,NA))))

#####just count
model.long.phase2.team<-  lmer(value.dif~domain*team_size*Month0+(1|domain/ResponseId), data=dat_long_phase2)
car::Anova(model.long.phase2.team,type="III") #nothing
summ(model.long.phase2.team, digits=4, center=T)
emtrends(model.long.phase2.team,specs=pairwise~domain,var="team_size") #larger team size linked to less bias for east asian explicit bias, but not any other domain
emtrends(model.long.phase2.team,~Month0|domain,var="team_size", at=list(Month0=c(0,5,11))) #
