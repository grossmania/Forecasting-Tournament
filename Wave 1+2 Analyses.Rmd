...
##IF WE WANT TO SORT AND GET TOP 5 
...
arrange(domain,MASE1_w1) %>%group_by(domain) %>% dplyr::slice_head(n = 5) %>% select(team_name,MASE1_w1,domain,team_size.coded,is_multidisciplinary,previous_tournament,Method.code,confidence,subexpert,numpred,Method.complex,team_expertise)

  
top.5.MASE.t2<-academic_only %>%filter(!(phase == 1 & revised == 1)) %>%   
arrange(domain,MASE1_w2) %>%group_by(domain) %>% dplyr::slice_head(n = 5) %>% select(team_name,MASE1_w2,domain,team_size.coded,is_multidisciplinary,previous_tournament,Method.code,confidence,subexpert,numpred,Method.complex,team_expertise)
  
proportions(xtabs( ~ domain+Method.code,top.5.MASE.t2),"domain")*100
...
dat_long %>% filter(!(phase == 1 & revised == 1)& !is.na(Method.code)& Method.code!="Naive-rfw" & Month %in% c(7,8,9,10,11,12)) %>% 
...
emmeans(model.long.phase2,specs = trt.vs.ctrl ~Method.code|domain, adjust = "none") #overall month (half a year estimate), contrast to intuition/theory. Intuition/thoery sig worse for eafric, easian, but better for neg affect,  
...
data.long.phase2.abs.dev %>% 
 ...
 geom_pointrange(aes(ymin=lower.CL, ymax=upper.CL), position=pd)+  theme_minimal(base_size = 14) +
theme(legend.position="bottom") +scale_color_d3()+scale_fill_d3()+ facet_wrap(~domain, scales = "free", nrow = 4, labeller=labeller(domain=labels))+
...
academic_only$Group[academic_only$TournamentStart=="May"&)


model.phase2.update<-  lmer(MASE1_w2~domain*Group+(1|ResponseId), data=academic_only)
...
## examine if phase 1 vs. phase 2
dat_long_phase2.full<-dat_long %>%filter(!is.na(Method.code)& isExpert.factor=="Academic"&Method.code!="Ground Truth"& Method.code!="Naive-linear"&Method.code!="Naive-rfw" & Month %in% c(7,8,9,10,11,12))

model.long.phase2.allphase<-  lmer(value.dif~domain*Method.code*Month*TournamentStart +(1|domain/ResponseId), data=dat_long_phase2.full,REML = F, control = lmerControl(optimizer ="Nelder_Mead"))
car::Anova(model.long.phase2.allphase,type="III") #phase matters, Method matters, and interactions

summ(model.long.phase2.allphase, digits=4) #to get R2
emmeans(model.long.phase2.allphase,specs = trt.vs.ctrl ~Method.code|TournamentStart|domain, adjust = "none") #overall month (half a year estimate), contrast to intuition/theory. Intuition/thoery sig worse for eafric, easian, but better for neg affect,  
emmeans(model.long.phase2,specs = trt.vs.ctrl ~Method.code|domain|Month0, adjust = "none", at=list(Month0=c(0,5))) #overall month (half a year estimate), contrast to intuition/theory. Lay ppl sig worse for eafric
